...
public class MABouncingBox {
     ...
     * with startColor.
     */
    public MABouncingBox(int startX, int startY, Color startColor) {
        x = startX;
        y = startY;
        color = startColor;
    }
    ...
    public void draw(Graphics surface) {
        ...
        if ((x - SIZE/2 <= 0 && xDirection < 0) ||
                (x + SIZE/2 >= MASimpleDraw.WIDTH && xDirection > 0)) {
            xDirection = -xDirection;
        }
        if ((y - SIZE/2 <= 0 && yDirection < 0) ||
                (y + SIZE/2 >= MASimpleDraw.HEIGHT && yDirection > 0)) {
